...
router.route("/geteventbypsnid/:psn_id").get((request, response) => {
  dboperations
    .getEventByPSNId(request.params.psn_id)
    .then((result) => {
      response.json(result);
    })
    .catch((err) => {
...
router.route("/getmanagebookdata/:psn_id").get((request, response) => {
  dboperations
    .getManageBookData(request.params.psn_id)
    .then((result) => {
      response.json(result);
    })
    .catch((err) => {
...
router
  .route("/deleteevent/:data_id/:personnel_id")
  .delete((request, response) => {
    dboperations
      .deleteEvent(request.params.data_id, request.params.personnel_id)
      .then((result) => {
        response.status(200).json(result);
      })
      .catch((err) => {
...
router.route("/getshiftbyid/:id").get((request, response) => {
  dboperations
    .getShiftById(request.params.id)
    .then((result) => {
      response.status(200).json(result);
    })
    .catch((err) => {
...
router.route("/getpsneventlist/:id").get((request, response) => {
  dboperations
    .getPSNEventList(request.params.id)
    .then((result) => {
      response.status(200).json(result);
    })
    .catch((err) => {
